...
int app_video_open(char *dev, video_fmt_t init_fmt)
{
    ...
    if (default_format.fmt.pix.pixelformat != (uint32_t)init_fmt) {
    ...
    if (ioctl(fd, VIDIOC_S_EXT_CTRLS, &ctrl) != 0) {
        ESP_LOGW(TAG, "failed to set vflip");
    }
...
static inline esp_err_t video_receive_video_frame(int video_fd)
{
    ...
    if (ioctl(video_fd, VIDIOC_DQBUF, &(app_camera_video.v4l2_buf)) != 0) {
...
esp_err_t app_video_stream_task_start(int video_fd, int core_id)
{
    ...
    BaseType_t result = xTaskCreatePinnedToCore(video_stream_task, "video stream task", VIDEO_TASK_STACK_SIZE, &app_camera_video.video_fd,
...
{
    return xSemaphoreTake(app_camera_video.video_stop_sem, portMAX_DELAY) == pdTRUE ? ESP_OK : ESP_FAIL;
}
